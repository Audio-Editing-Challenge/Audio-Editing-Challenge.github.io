...
1. **Reproducibility.** Teams must provide complete inference code and configurations. Finalists must additionally provide the training recipe and model weights or a stable, research-permissive checkpoint link for verification.
2. **No external models, tools, or APIs.** Separate ASR, source separation, captioning, retrieval, planning, or editing systems are prohibited at inference, as are commercial or hosted APIs.
3. **No manual post-processing.** Submitted audio must be generated by the model. Only deterministic resampling, channel conversion, normalization, and file-format conversion are permitted after generation.
